...
; The file to write
OutFile "texgen-bundle-64bit-Python312-${VERSION}.exe"
...
Section "TexGen (required)" ;No components page, name is not important
 ...
 ; File msvcr71.dll
  ...
  ClearErrors
  ReadRegStr $0 HKLM "SOFTWARE\Python\PythonCore\3.12\PythonPath" ""
  ${If} ${Errors}
    WriteRegStr HKLM "SOFTWARE\Python\PythonCore\3.12\PythonPath" "" "$INSTDIR\Python\libstd"
  ${Else}
    ${IF} $0 == ""
             WriteRegStr HKLM "SOFTWARE\Python\PythonCore\3.12\PythonPath" "" "$INSTDIR\Python\libstd"
        ${ELSE}
               DetailPrint "Python 312 path already exists"
        ${ENDIF}
  ${EndIf}
